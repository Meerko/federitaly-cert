'use client';
import { Factory, Handshake, Sprout, ShieldUser } from 'lucide-react';
    ...
    subitems: [
      {
        label: 'Made in Italy',
        href: '/made-in-italy',
        description: 'Certificazione dell’origine',
        icon: Factory,
      },
      {
        label: '100% Made in Italy',
        href: '/100-made-in-italy',
        description: 'Il grado più elevato di certificazione',
        icon: Handshake,
      },
    ],
  },
  { label: 'Federitaly', href: '/#federitaly' },
    ...
    >
      <div
        className={cn(
          ...
          isScrolled &&
            isAtLeast('lg') &&
            'h-[calc(var(--header-height)-20px)] max-w-7xl rounded-xl shadow-sm backdrop-blur-md',
        )}
      >
        <Logo className="" />
                        ...
                        <ul className="grid w-[263px] gap-2">
                          {item.subitems.map((subitem) => (
                            <li key={subitem.label}>
                              <NavigationMenuLink
                                href={subitem.href}
                                className="hover:bg-primary/10 flex-row gap-3 p-3"
                              >
        ...
        <div
          className={cn(
            'bg-background/95 text-foreground fixed inset-0 -z-10 flex flex-col justify-between tracking-normal backdrop-blur-md transition-all duration-500 ease-out lg:hidden',
            isBannerVisible
        ...
        >
                            ...
                            <div className="space-y-2">
                              {item.subitems.map((subitem) => (
                                <NavigationMenuLink
                                  ...
                                  className={cn(
                                    'text-muted-foreground hover:bg-primary/10 flex flex-row gap-2 p-3 font-medium transition-colors',
          ...
          <div className="flex flex-col gap-4.5 border-t px-6 py-4">
            {ACTION_BUTTONS.map((button) => (
              ...
              >
                <Link href={button.href} target="_blank" onClick={() => setIsMenuOpen(false)}>
                  {button.label}
                </Link>
              </Button>
            ))}
